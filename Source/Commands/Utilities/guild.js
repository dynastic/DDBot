  ...
  (client, message, response, args) => {
    var operations = {
      moduleList (config) {
        ...
        response.reply('', response.embedFactory.createInformativeEmbed('Available Modules').setDescription(content))
      },
      moduleToggle (config) {
        if (!args[1]) {
          response.reply('', response.embedFactory.createErrorEmbed().setDescription(`Usage: \`${client.config.prefx}guild moduleToggle <module ID>\``))
          return
        }
        ...
        config.save()
        message.guild.manager.properties = config
        response.reply('', response.embedFactory.createSuccessEmbed().setTitle('Output').setDescription(content))
      },
      commandList (config) {
    ...
    GuildConfig.findOne({snowflake: message.guild.id}, (err, doc) => {
      ...
      if (!operations[args[0]]) {
        ...
        response.reply('', response.embedFactory.createInformativeEmbed('ModuleManager Commands').setDescription(content))
        return
      }
