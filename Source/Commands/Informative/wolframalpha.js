...
module.exports = new Command("wolframalpha", "Submit your query to Wolfram Alpha", "<query>", ["alpha", "wolfram", "query", "wa"],
    (client, message, response, args) => {
        ...
        let query = args.join(" ");
        let headers = {"User-Agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_12_4) AppleWebKit/603.1.30 (KHTML, like Gecko) Version/10.1 Safari/603.1.30", "Origin": "https://products.wolframalpha.com", "Referer": "https://products.wolframalpha.com/api/explorer/", "Accept": "*/*"};
        response.reply("", response.embedFactory.createInformativeEmbed("Retrieving result...").setDescription("Contacting Wolfram Alpha..."), true).then(postedMessage => {
            rp({url: 'https://www.wolframalpha.com/input/apiExplorer.jsp?input=' + encodeURIComponent(query) + '&format=image,plaintext&output=JSON&type=full&width=200', headers: headers}).then(body => {
                let result = JSON.parse(body).queryresult;
                let url = `http://www.wolframalpha.com/input/?i=${encodeURIComponent(query)}`;
                if(!result.success) {
                    let embed = response.embedFactory.createErrorEmbed("No results").setDescription(`Wolfram Alpha returned no results for your query of *${query}*.`).setURL(url);
                    if(result.tips) {
                        embed.addField("Tips", result.tips.text);
                    }
                    return response.edit(postedMessage, postedMessage.content, embed);
                }
                let embed = response.embedFactory.createSuccessEmbed("Results").setDescription(`Wolfram Alpha results for *${query}*:` + (result.pods.length > 6 ? `\n*Some results were truncated. [Click here](${url}) to view the full results.*` : "")).setURL(url);
                if(result.pods) {
                    result.pods.slice(0, 6).forEach(pod => {
                        let text = pod.subpods.map(subpod => {
