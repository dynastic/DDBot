...
module.exports = new Command('case', 'View a moderator action case', '<case number>', ['viewcase', 'caselookup', 'lookupcase', 'cl', 'vc', 'caseinfo', 'viewaction', 'actionlookup'],
    (client, message, response, args) => {
      ...
      }).catch(err => response.reply('', response.embedFactory.createErrorEmbed("Couldn't find case", message.member).setDescription('A case with the specified number could not be found for this server.')))
    }, ['MANAGE_GUILD']
)
