  ...
  (client, message, response, args) => {
    ...
    Case.createCase(message.author, 'liftwarn', user, message.guild, null, -points).then(newCase => {
      UserInfo.addWarningPoints(user, message.guild, -points).then(userInfo => {
        ...
        client.log(err, true)
        response.reply('', response.embedFactory.createErrorEmbed(null, message.member).setDescription(`An unknown error occurred while trying to lift warns from the user.`))
      })
    }).catch(err => {
      client.log(err, true)
      response.reply('', response.embedFactory.createErrorEmbed(null, message.member).setDescription(`An unknown error occurred while trying to lift warns from the user.`))
