...
module.exports = new Command('nick', 'Set a nickname', '[user] [nickname]', ['setnick', 'setnickname', 'nickname'],
  (client, message, response, args) => {
    ...
    var canChangeOtherNicknames = message.member.hasPermission('MANAGE_NICKNAMES'), canChangeOwnNickname = message.member.hasPermission('CHANGE_NICKNAME')
    ...
    var validUser = user && modUtils.validateUserArgument(user, args[0])
    if (!validUser) { user = message.author }
    var nick = args[validUser ? 1 : 0] || ''
