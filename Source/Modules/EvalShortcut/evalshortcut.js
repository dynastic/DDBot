class EvalShortcut {
  constructor (client) {
    ...
    this.events = {
      message: (message) => {
        if (message.content === this.config.prefix || !message.content.startsWith(this.config.prefix) || message.content.startsWith(this.config.prefix + this.config.prefix) || !this.client.config.admins.includes(message.author.id)) return;
